...
2. Create top-level folders `Docs`, `.github`, `.venv`, `tests`, `ComfyUI-Xtremetools` in `c:\nodedev`.
3. Document mirror requirement: plan how `ComfyUI-Xtremetools` will sync to `C:\ComfyUI_windows_portable\ComfyUI\custom_nodes\ComfyUI-Xtremetools` (e.g., script or symlink).
...
9. Mirror strategy symlink: Use a symbolic link to connect `ComfyUI-Xtremetools` in the workspace to the ComfyUI custom nodes directory. This allows for real-time synchronization of changes made in the workspace with the ComfyUI installation.