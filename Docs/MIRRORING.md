...
## Steps
1. Delete any existing `Xtremetools` folder inside the ComfyUI `custom_nodes` directory (back it up first if needed). Emoji/non-ASCII folder names cause ComfyUI discovery to fail, so keep the folder ASCII-only.
